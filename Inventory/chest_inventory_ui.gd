...
func update_slots():
	for i in range(min(inventory.slots.size(), slots.size())):
		var inventorySlot: InventorySlot=inventory.slots[i] #click
...
func takeItemFromSlot(slot):
	...
	if itemInHand:  # itemInHand'in null olup olmadığını kontrol edin
...
func insertItemInSlot(slot):
	var item = itemInHand
	if item:  # item'in null olup olmadığını kontrol edin
